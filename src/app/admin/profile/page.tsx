...
export default function AdminProfilePage() {
  ...
  useEffect(() => {
    ...
    if (!loading && !user) {
      ...
      const docRef = doc(getFirebaseDb(), "admins", user.uid);
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      localStorage.removeItem(`adminProfile_${user.uid}`);
      ...
      if (
        ...
        setAuthError(
          "Deleting your account requires a recent login. Please log out and log back in, then try again.",
        );
      } else {
  ...
  const handleUpdateAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (
        ...
        setAuthError(
          "This action requires a recent login. Please provide your current password above or log out and log back in.",
        );
      } else {
        setAuthError(
          err instanceof Error
            ? err.message
            : "Error updating authentication details.",
        );
      }
    } finally {
  ...
  const handleLinkProvider = async (
    ...
    try {
      ...
      if (err.code === "auth/credential-already-in-use") {
        setAuthError(
          `This ${providerName} account is already linked to another profile.`,
        );
    ...
    <div className="min-h-screen relative overflow-x-hidden pt-24 pb-16 px-4">
      <div className="container relative z-10 mx-auto max-w-3xl">
        ...
        <div className="glass-card card-3d p-5 md:p-8 rounded-2xl md:rounded-3xl shadow-lg border border-primary/10 mb-8 backdrop-blur-xl bg-background/40 card-3d">
          <h2 className="text-2xl font-bold tracking-tight border-b border-border/50 pb-3 mb-4 text-foreground/90">
            Account Credentials
          </h2>
          {authError && (
          ...
          <div className="mt-8 pt-6 border-t border-border/50">
            ...
            <p className="text-sm text-muted-foreground mb-4">
              Link additional providers to sign in with any of them.
            </p>
