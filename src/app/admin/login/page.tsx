...
export default function AdminLogin() {
  ...
  const verifyAndBootstrapAdmin = async (user: User) => {
    ...
    if (
      ...
      const result = await bootstrapSuperAdmin(idToken);
      if (result.success) {
    ...
    if (!authorized) {
      ...
      throw new Error(
        "Unauthorized. Only administrators can access this portal.",
      );
    }

    if (needsTokenRefresh) {
      // Force refresh the token to propagate the new custom claim to the frontend immediately
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await verifyAndBootstrapAdmin(userCredential.user);
      router.push("/admin");
    } catch (err: any) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[70vh] px-4 pt-16">
