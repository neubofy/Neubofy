...
import { X, Apple, Github, ArrowRight, ShieldCheck, Sparkles } from "lucide-react";
...
function OnboardContent() {
  ...
  const initializePartnerDoc = async (uid: string, email: string, displayName?: string, photoURL?: string) => {
    ...
    try {
      if (executeRecaptcha) {
        try {
          ...
          if (!isValid) {
            console.warn("reCAPTCHA validation flagged low score, proceeding in development.");
          }
        } catch (captchaErr) {
          console.warn("reCAPTCHA check bypassed:", captchaErr);
        }
      }
      ...
      <div className="relative z-10 w-full max-w-md p-8 rounded-2xl glass-card card-3d border border-white/10 backdrop-blur-2xl">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Button 
            type="submit" 
            disabled={loading} 
            className="w-full h-12 btn-electric rounded-xl font-medium mt-2 gap-2"
          >
